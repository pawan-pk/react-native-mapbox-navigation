 ...
 */
  ...
  init {
    onCreate()
    eventDispatcher = UIManagerHelper.getEventDispatcherForReactTag(context, id)
  }
    ...
    if (leg != null) {
      eventDispatcher?.dispatchEvent(
        ArriveEvent(
          ...
          null,
          leg.legIndex,
          leg.legDestination?.location?.latitude() ?: 0.0,
          leg.legDestination?.location?.longitude() ?: 0.0
        )
      )
    }
  }
